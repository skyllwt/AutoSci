...
> 覆盖 `runtime/schema/entities.yaml` 中声明的所有 entity 类型（papers, concepts, topics, people, ideas, experiments, methods, foundations, manuscripts, reviews），以及 graph edge / citation 一致性。重点检查包括：idea novelty-score 合理性、idea 失败原因完整性、experiment `linked_idea` 有效性。
...
- `wiki/methods/*.md` — method type、source_papers、parent/child 链、grounded_in、parent_topics
- `wiki/foundations/*.md` — foundation 字段 + grounding/contribution 反向 section(不再是终端)
- `wiki/manuscripts/*.md` — manuscript status、slug、tags、`## Reviews` 反向
