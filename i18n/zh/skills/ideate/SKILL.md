...
1. **Primary agent 生成 6-10 个 ideas**：
   ...
   - **结构化生成路径** — 每个 idea 必须遵循以下四条路径之一：
     ...
     |------|------|------------------|----------|
     | A | 景观驱动 (Landscape-driven) | `direction` + Phase 1 景观报告（不依赖已有方法） | "基于 topicresearch 描述直接设计" |
...
1. **写入 top ideas**（status: proposed）：
   ...
   **Priority 计算**（把 Phase 3 验证信号映射到 1-5 分）：
   - 若 `--skip-validation`：默认 `priority = 3`（跳过 noveltyreview 评分）
   ...
   **正文结构**（必须与 `runtime/templates/ideas.md.tmpl` 严格一致 — 不要改名）：
   ...
   ## Pilot results
   （留空 — 由 exp-pilot-run和exp-pilot-eval 后填写）
...
| 路径 | 预实验方式 |
|------|-----------|
| A (景观驱动) | 基于 topicresearch 描述直接实现提出的方法。在小规模 benchmark 上运行，验证 idea 可行且产生非退化输出。与简单 baseline 对比。 |
...
- `task` tool — Phase 1 并行搜索、Phase 2 并行脑暴
