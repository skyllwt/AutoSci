---
name: init
description: 基于用户素材与可选外部发现搭建 AutoSci，并通过 Codex-safe 串行路径或可选并行 worktree 路径完成最终论文集 ingest
...
- `references/parallel-ingest.md` — Codex-safe 串行 ingest 合同，以及可选 worktree 隔离、子代理 prompt 合同、merge 与清理
...
- ingest workflow 产出的最终论文页面（Codex 默认串行；可选并行 worktree）
...
- `.checkpoints/init-prepare.json` 与 `.checkpoints/init-sources.json`，供 resume、planning 与 batch ingest 使用
...
- `.checkpoints/init-prepare.json`、`.checkpoints/init-plan.json`、`.checkpoints/init-sources.json`、`.checkpoints/init-handoff.json` 与 `init-session` checkpoint metadata
...
然后生成有序 ingest handoff。Codex 应使用 `--mode serial`；支持并行的 runtime 可在 Step 5 前用 `--mode parallel` 重新生成：
...
本步骤的论文来源只能来自 `.checkpoints/init-handoff.json`；该文件由 `.checkpoints/init-sources.json` 派生：
...
- 不创建 worktree
- 不要求当前位于命名分支
- 不仅为了 batch ingest 而 stash 无关脏文件；但仍不得编辑无关文件
- 按 `.checkpoints/init-handoff.json` 中的 `tasks[*].order` 顺序处理任务
- 每个任务先读取 `tasks[*].active_ingest_skill` 指定的当前 runtime active ingest skill 指令，然后只对 `tasks[*].canonical_ingest_path` 执行一次 ingest workflow；不得绕过 ingest 直接手写 paper/concept/method 页面
- 在 handoff 中明确写出 **INIT MODE SERIAL**
- INIT MODE SERIAL 下：
  - 必须原样消费 canonical path
  - `raw/` 只读
  - 跳过 `fetch_s2.py citations`
  - 跳过 `fetch_s2.py references`
  - 跳过每篇论文自己的 `rebuild-index`
  - 跳过每篇论文自己的 `rebuild-context-brief`
  - 跳过每篇论文自己的 `rebuild-open-questions`
  - 跳过易冲突 topic 写入
  - 跳过每篇论文自己的 visualization 与 discovery follow-up
  - 每篇论文结束后**不**创建 git commit
- 若某篇论文失败，将失败写入最终报告；只在工作区状态仍清晰时继续处理其余论文。若 partial failure 留下含糊 wiki 状态，停止并报告恢复点
- 所有 rebuild、dedup、lint、citation backfill 与 visualization 都在 Step 6 中 batch 完成后统一执行一次
...
- 串行 fallback、worktree 命令、merge 顺序、fan-in 与清理见 `references/parallel-ingest.md`

### Step 6: fan-in、rebuild 与最终报告

串行 ingest 完成后，或全部并行子代理完成后：

- 串行模式下不 merge branch，继续使用主工作区
- 并行 worktree 模式下，在 `BASE_BRANCH` 上按顺序 merge worktree branches
...
若并行模式创建了 `stash_ref`，在最后再 pop。若 stash pop 失败，保留 checkpoint 并在报告中说明。
...
- Step 5 必须读取 `.checkpoints/init-handoff.json`，不得临时扫描目录
