...
> 手动：Codex 中 `$exp-eval <experiment-slug>`，或 Codex 中 `$exp-eval <experiment-slug>`。
...
3. **建议下一步**：Codex 的 `$paper-plan {linked-idea}` 或 Codex 的 `$paper-plan {linked-idea}`，或继续 ablation/robustness 实验
...
3. **建议补充实验**：
   - 明确缺少什么证据
   - 建议用 Codex 的 `$exp-design --linked-idea {linked-idea}` 或 Codex 的 `$exp-design --linked-idea {linked-idea}` 设计补充实验
...
3. **建议下一步**：
   ...
   - 建议用 Codex 的 `$ideate` 或 Codex 的 `$ideate` 生成替代方案
...
- **只处理 completed 实验**：status != completed 的实验拒绝处理，提示先用 Codex 的 `$exp-run` 或 Codex 的 `$exp-run` 完成。
...
- **`linked_idea` 缺失**：拒绝继续；提示用户运行 Codex 的 `$edit` 或 Codex 的 `$edit` 设置实验的 `linked_idea`。
...
- **结果数据缺失**：若实验页面的 Results section 为空，提示用户先运行 Codex 的 `$exp-run {slug} --check` 或 Codex 的 `$exp-run {slug} --check`。
...
### Called by
- `$research`（Codex）或 `$research`（Codex）Stage 4（判决与迭代阶段）
- 用户手动调用
