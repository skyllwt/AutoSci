...
> 不修改任何 wiki 页面。不判定 pass/fail — 结果由 Codex 的 `$exp-pilot-eval` 或 Codex 的 `$exp-pilot-eval` 评估。
...
### Reads
- `experiments/pilot/{slug}.yaml` — Pilot Spec（所有配置）。如果缺失，报告错误并建议先运行 Codex 的 `$ideate` 或 Codex 的 `$ideate` 生成；不要在本 skill 中创建 spec。
...
1. **读取 Pilot Spec**：
   如果 `experiments/pilot/{slug}.yaml` 缺失，报告错误并建议先运行 Codex 的 `$ideate` 或 Codex 的 `$ideate` 生成。不要在这里合成或创建 Pilot Spec；`$exp-pilot-run` / `$exp-pilot-run` 只执行已有 spec。
...
6. **输出 PILOT_REPORT 到终端**：
   ...
   ## Next Steps
   - 进入 Codex 的 `$exp-pilot-eval` 或 Codex 的 `$exp-pilot-eval` 进行判定评估和 wiki 更新
   ```
...
- **Pilot Spec 缺少字段**：报告缺失的必填字段，建议重新运行 Codex 的 `$ideate` 或 Codex 的 `$ideate`
