...
只要 `ingest` 的来源路径来自 `.checkpoints/init-sources.json`，就处于 INIT MODE。上层 `init` 会为每篇论文在隔离的 `git worktree` 内跑一个 `ingest`，流程见 `skillsinit/references/parallel-ingest.md`。
