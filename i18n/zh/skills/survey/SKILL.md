...
4. **生成候选论文列表**：
   ...
   - 限制为 `--max-papers` 篇
5. **若候选论文 < 5 篇**：警告「相关论文不足，建议先用 Claude Code 的 `/ingest` 或 Codex 的 `$ingest` 增加论文」
...
- **候选论文 < 5 篇时警告**：提示用户先用 Claude Code 的 `/ingest` 或 Codex 的 `$ingest` 增加论文
...
- **wiki 论文不足 3 篇**：报错，建议先用 Claude Code 的 `/ingest` 或 Codex 的 `$ingest` 增加足够数量的论文
...
### MCP Servers
- 无（survey 不需要 Review LLM，可通过 Claude Code 的 `/review --focus writing` 或 Codex 的 `$review --focus writing` 单独审查）
...
### Called by
- `/paper-draft`（Claude Code）或 `$paper-draft`（Codex）Step 3（Related Work section 可委托此 skill）
- 用户手动调用
