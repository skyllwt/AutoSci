...
8. **输出DESIGN_REPORT到终端**：
   ...
   ## 下一步
   - 运行 Codex 的 `$exp-run [[sensitivity-slug]]` 或 Codex 的 `$exp-run [[sensitivity-slug]]` 开始 Stage 0
   ```
...
### Skills
- `$exp-run`（Codex）或 `$exp-run`（Codex）— 执行设计好的实验
- `$exp-pilot-eval`（Codex）或 `$exp-pilot-eval`（Codex）— 前置条件：预实验评估应在正式设计前完成
...
### Called by
- `$ideate`（Codex）或 `$ideate`（Codex）预实验评估后
- 用户直接调用
