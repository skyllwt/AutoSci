...
2. **自动恢复检测**（无 `--start-from` 时）：
   - 若 `wiki/outputs/pipeline-progress.md` 存在 且 `status == running`：
     ...
     - 若用户选 [3]：在 Codex 中调用 `$exp-status --pipeline {slug}`，或在 Codex 中调用 `$exp-status --pipeline {slug}`，然后退出，不继续执行
...
**触发条件**：用户在 Codex 中手动运行 `$research --start-from stage3-collect`，或在 Codex 中运行 `$research --start-from stage3-collect`
...
### Skills
- `$ingest`（Codex）或 `$ingest`（Codex）— Stage 0 Bootstrap auto-ingest
- `$ideate`（Codex）或 `$ideate`（Codex）— Stage 1 idea 发现
- `$exp-design`（Codex）或 `$exp-design`（Codex）— Stage 2 实验设计
- `$exp-run`（Codex）或 `$exp-run`（Codex）— Stage 3a（deploy 模式）和 Stage 3c（--collect 模式）
- `$exp-status`（Codex）或 `$exp-status`（Codex）— 用户手动查看实验进度；`--auto-advance` 可在 agent runtime 内继续 Stage 4，外部调度器只报告 `stage4 ready` 并输出 `$research --start-from stage4`
- `$exp-eval`（Codex）或 `$exp-eval`（Codex）— Stage 4 判决
- `$refine`（Codex）或 `$refine`（Codex）— Stage 4 迭代 + Stage 5 论文改进
- `$paper-plan`（Codex）或 `$paper-plan`（Codex）— Stage 5 论文规划
- `$paper-draft`（Codex）或 `$paper-draft`（Codex）— Stage 5 论文撰写
- `$paper-compile`（Codex）或 `$paper-compile`（Codex）— Stage 5 论文编译
