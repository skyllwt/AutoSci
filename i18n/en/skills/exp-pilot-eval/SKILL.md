...
> Called by `$ideate` in Codex or `$ideate` in Codex after `$exp-pilot-run` / `$exp-pilot-run` completes.
...
3. Print **PILOT_VERDICT_REPORT** to terminal and write to `experiments/pilot/{slug}/report.md`:
   ...
   ## Next Steps
   - {if pass: proceed to `$exp-design` in Codex or `$exp-design` in Codex for full experiments}
...
- **Idea page not found**: report error, suggest running `$ideate` in Codex or `$ideate` in Codex first
- **Pilot results not found**: report error, suggest running `$exp-pilot-run` in Codex or `$exp-pilot-run` in Codex first
