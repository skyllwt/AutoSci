...
- **Top result is a foundation with score ≥ 0.40** — route to foundation linking. The candidate is textbook background, not a new mechanism. Write a `paper → foundation` edge of type `contributes_to_foundation` (with `--confidence`/`--evidence`); its reverse goes to the foundation's `## Contributed by papers` section (append in the same turn, or run `lint --fix`). Foundations are no longer terminal — see `references/cross-references.md`. Foundation links do not count against the per-paper creation limit.
