...
INIT MODE is active for any `ingest` invocation whose source path originates from `.checkpoints/init-sources.json`. The parent `init` runs one `ingest` per paper in an isolated `git worktree`, following the contract in `skillsinit/references/parallel-ingest.md`.
