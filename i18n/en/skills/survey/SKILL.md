...
5. **If candidate papers < 5**: warn "insufficient related papers; consider `$ingest` in Codex or `$ingest` in Codex for more papers first"
...
- **Warn when candidate papers < 5**: prompt user to run `$ingest` in Codex or `$ingest` in Codex for more papers first
...
- **Fewer than 3 wiki papers**: error; suggest `$ingest` in Codex or `$ingest` in Codex for enough papers first
...
### MCP Servers
- None (survey does not require Review LLM; use `$review --focus writing` in Codex or `$review --focus writing` in Codex for separate review)
...
### Called by
- `$paper-draft` (Codex) or `$paper-draft` (Codex) Step 3 (Related Work section can be delegated to this skill)
