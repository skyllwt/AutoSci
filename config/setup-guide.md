...
> This file is read by the OpenCode `setup` skill
...
- **Option A — Auto-register**: OpenCode can register a free token automatically.
...
- `daily-arxiv` — scheduled recommendation and optional high-confidence auto-ingest through the primary model or OpenCode; inform-only fallback when no coding-agent runtime is configured
