...
function showIntentModal(payload) {
  ...
  pop.querySelector("#intent-cmd").textContent = payload.codex_command || payload.command || "";
  const docEl = pop.querySelector("#intent-doc");
  if (payload.codex_doc_url) {
    docEl.innerHTML = `Skill spec: <code>${escHtml(payload.codex_doc_url)}</code>`;
    docEl.style.display = "";
  } else {
