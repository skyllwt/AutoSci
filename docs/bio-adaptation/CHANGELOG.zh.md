...
## 2026-05-12 —— B-infra pilot merge：typed metadata nested-schema 验证收尾

**范围**：B1 / B2 / B3 早期 minimal pilot 把 typed metadata 验证标为延后("还需 add-edge CLI
扩展 + loader 验证")。a3a2751 已加 add-edge --metadata。本次给 loader 加 metadata schema
验证 + 给 5 个边类型声明 metadata 子 schema,关闭 B 节最后一项延后。
**状态**：**B-infra 合并**(完整)。schema 定义 + loader 验证 + 现有 legacy edge 迁移 +
end-to-end smoke 全部通过。Section B 此后无遗留。

### 改动位置

- `runtime/schema/edges.yaml`：5 个边类型加 `metadata:` 子块（与 `attributes:` 并列）:
  ...
  - `clinical_trial_for.metadata`：`nct_id str`、`phase enum [0,1,1/2,2,2/3,3,4]`、`indication str`、`year int`
  - `fda_approved_for.metadata`：`indication str required`、`year int`、`approval_kind enum [standard,accelerated,breakthrough,fast-track]`
  - `validates_in_species.metadata`：`species str required`、`source_db str`
  - 其他 9 个 B1 动词（targets_protein、ubiquitinates、inhibits、activates、degrades、
    phosphorylates、methylates、acetylates、is_substrate_of）保持无 metadata 块 → legacy
    passthrough（接受任意 metadata）。
- `runtime/loader.py::validate_edge_attributes`：扩展到验证 metadata
  - closed-set:未声明 key 触发 `edge-attribute` 警告,带 "known keys" 提示
  - type 检查（enum / int / float / str）
  - required-key 检查 —— 但**仅当 metadata 块在 edge 上存在时**（block 整体缺失视为 legacy 未迁移,通过）
  - 与现有 attributes 验证共享同一 errors 列表 + 同一 lint.py 调用路径
- **Live edge 迁移**：B3 minimal pilot 时代留下的单条 `dataset_version_used` 边（版本编码在
  evidence 字符串）迁移到 typed 形态:`metadata.version=v1`、`metadata.subset=crbn-vhl-test`;
  evidence 字符串去除冗余 "version: v1" 前缀。
...
|---|---|
| `python tools/lint.py` | 0 🔴 / 0 🟡 / 11 🔵（不变） |
| `python tools/lint_bio.py` | 0 🔴 / 0 🟡 / 0 🔵（不变） |
| Smoke：3 个 live typed-metadata edge | 全部 validate 通过 |
| Smoke：6 个 bad case | missing required / unknown key / invalid enum / bad int / bad float / non-dict —— 全部正确 flagged |
| End-to-end lint inject smoke（TYPO_KEY） | lint.py 命中 `edge-attribute` 警告,带 "known keys" 提示 |
| Live bio relation edge typed metadata 覆盖 | 4/7（升自 3/7） |

**Section B infra 关闭**。Section A + B + C + D backlog 全部子项至此合并完成。
