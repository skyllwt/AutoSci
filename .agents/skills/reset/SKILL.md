---
name: reset
description: "AutoSci /reset Codex adapter. Use when the user invokes `/reset` or asks Codex to run AutoSci's reset workflow. Upstream behavior: Reset wiki state to a clean scaffold by scope (wiki / raw / log / checkpoints / all). Useful during development or carefree restarts after a botched setup."
---

# AutoSci /reset (Codex Adapter)
...
When this skill is selected, first read `.claude/skills/reset/SKILL.md` completely, then follow
that workflow as the source of truth.

Use these Codex-specific adaptations while following the upstream instructions:

- Treat AutoSci's original slash command `/reset` as this Codex skill.
  If the user writes `/reset ...`, execute it through this skill.
- Resolve any relative references from the upstream skill directory
  `.claude/skills/reset`. For example, `references/foo.md` means
  `.claude/skills/reset/references/foo.md`.
...
- Upstream skill: `.claude/skills/reset/SKILL.md`
