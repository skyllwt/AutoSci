---
name: exp-pilot-run
description: "AutoSci /exp-pilot-run Codex adapter. Use when the user invokes `/exp-pilot-run` or asks Codex to run AutoSci's exp-pilot-run workflow. Upstream behavior: Pilot experiment execution — read Pilot Spec YAML, write pilot code, run experiment(Confirm with the user before operation and require the applicant to conduct manual inspection), return results. Called by /ideate Phase 5. Does NOT modify wiki pages or judge pass/fail."
---

# AutoSci /exp-pilot-run (Codex Adapter)
...
When this skill is selected, first read `.claude/skills/exp-pilot-run/SKILL.md` completely, then follow
that workflow as the source of truth.

Use these Codex-specific adaptations while following the upstream instructions:

- Treat AutoSci's original slash command `/exp-pilot-run` as this Codex skill.
  If the user writes `/exp-pilot-run ...`, execute it through this skill.
- Resolve any relative references from the upstream skill directory
  `.claude/skills/exp-pilot-run`. For example, `references/foo.md` means
  `.claude/skills/exp-pilot-run/references/foo.md`.
...
- Upstream skill: `.claude/skills/exp-pilot-run/SKILL.md`
