---
name: survey
description: "AutoSci /survey Codex adapter. Use when the user invokes `/survey` or asks Codex to run AutoSci's survey workflow. Upstream behavior: Generate a Related Work section for a paper from wiki knowledge — thematic grouping → narrative structure → LaTeX output, following citation-verification and academic-writing"
---

# AutoSci /survey (Codex Adapter)
...
When this skill is selected, first read `.claude/skills/survey/SKILL.md` completely, then follow
that workflow as the source of truth.

Use these Codex-specific adaptations while following the upstream instructions:

- Treat AutoSci's original slash command `/survey` as this Codex skill.
  If the user writes `/survey ...`, execute it through this skill.
- Resolve any relative references from the upstream skill directory
  `.claude/skills/survey`. For example, `references/foo.md` means
  `.claude/skills/survey/references/foo.md`.
...
- Upstream skill: `.claude/skills/survey/SKILL.md`
