---
name: paper-compile
description: "AutoSci /paper-compile Codex adapter. Use when the user invokes `/paper-compile` or asks Codex to run AutoSci's paper-compile workflow. Upstream behavior: LaTeX compile → PDF — latexmk compile + auto-fix + page count/anonymity/font/[UNCONFIRMED] checks + submission checklist"
---

# AutoSci /paper-compile (Codex Adapter)
...
When this skill is selected, first read `.claude/skills/paper-compile/SKILL.md` completely, then follow
that workflow as the source of truth.

Use these Codex-specific adaptations while following the upstream instructions:

- Treat AutoSci's original slash command `/paper-compile` as this Codex skill.
  If the user writes `/paper-compile ...`, execute it through this skill.
- Resolve any relative references from the upstream skill directory
  `.claude/skills/paper-compile`. For example, `references/foo.md` means
  `.claude/skills/paper-compile/references/foo.md`.
...
- Upstream skill: `.claude/skills/paper-compile/SKILL.md`
