...
- Polish with `$refine paper/main.tex --focus writing` in Codex or `$refine paper/main.tex --focus writing` in Codex
...
- **main.tex not found**: error; suggest running `$paper-draft` in Codex or `$paper-draft` in Codex first
...
### Called by
- `$research` (Codex) or `$research` (Codex) Stage 5 (paper compilation stage)
