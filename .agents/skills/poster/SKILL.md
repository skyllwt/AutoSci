---
name: poster
description: "AutoSci /poster Codex adapter. Use when the user invokes `/poster` or asks Codex to run AutoSci's poster workflow. Upstream behavior: Generate an academic poster from a drafted paper — distill sections into a single-page HTML poster with figures and inter-section transitions"
---

# AutoSci /poster (Codex Adapter)
...
When this skill is selected, first read `.claude/skills/poster/SKILL.md` completely, then follow
that workflow as the source of truth.

Use these Codex-specific adaptations while following the upstream instructions:

- Treat AutoSci's original slash command `/poster` as this Codex skill.
  If the user writes `/poster ...`, execute it through this skill.
- Resolve any relative references from the upstream skill directory
  `.claude/skills/poster`. For example, `references/foo.md` means
  `.claude/skills/poster/references/foo.md`.
...
- Upstream skill: `.claude/skills/poster/SKILL.md`
