---
name: exp-run
description: "AutoSci /exp-run Codex adapter. Use when the user invokes `/exp-run` or asks Codex to run AutoSci's exp-run workflow. Upstream behavior: Full experiment execution pipeline — prepare code → deploy(Confirm with the user before operation and ask the applicant to conduct manual inspection) → monitor → collect results, supporting three run modes"
---

# AutoSci /exp-run (Codex Adapter)
...
When this skill is selected, first read `.claude/skills/exp-run/SKILL.md` completely, then follow
that workflow as the source of truth.

Use these Codex-specific adaptations while following the upstream instructions:

- Treat AutoSci's original slash command `/exp-run` as this Codex skill.
  If the user writes `/exp-run ...`, execute it through this skill.
- Resolve any relative references from the upstream skill directory
  `.claude/skills/exp-run`. For example, `references/foo.md` means
  `.claude/skills/exp-run/references/foo.md`.
...
- Upstream skill: `.claude/skills/exp-run/SKILL.md`
