---
name: research
description: "AutoSci /research Codex adapter. Use when the user invokes `/research` or asks Codex to run AutoSci's research workflow. Upstream behavior: End-to-end research orchestrator — idea discovery → experiment design → execution → verdict → paper writing, with human gates and session-resumable state"
---

# AutoSci /research (Codex Adapter)
...
When this skill is selected, first read `.claude/skills/research/SKILL.md` completely, then follow
that workflow as the source of truth.

Use these Codex-specific adaptations while following the upstream instructions:

- Treat AutoSci's original slash command `/research` as this Codex skill.
  If the user writes `/research ...`, execute it through this skill.
- Resolve any relative references from the upstream skill directory
  `.claude/skills/research`. For example, `references/foo.md` means
  `.claude/skills/research/references/foo.md`.
...
- Upstream skill: `.claude/skills/research/SKILL.md`
