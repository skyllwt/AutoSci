---
name: prefill
description: "AutoSci /prefill Codex adapter. Use when the user invokes `/prefill` or asks Codex to run AutoSci's prefill workflow. Upstream behavior: Seed wiki/foundations/ with domain background knowledge so subsequent /ingest does not create duplicate concept pages for textbook material"
---

# AutoSci /prefill (Codex Adapter)
...
When this skill is selected, first read `.claude/skills/prefill/SKILL.md` completely, then follow
that workflow as the source of truth.

Use these Codex-specific adaptations while following the upstream instructions:

- Treat AutoSci's original slash command `/prefill` as this Codex skill.
  If the user writes `/prefill ...`, execute it through this skill.
- Resolve any relative references from the upstream skill directory
  `.claude/skills/prefill`. For example, `references/foo.md` means
  `.claude/skills/prefill/references/foo.md`.
...
- Upstream skill: `.claude/skills/prefill/SKILL.md`
