---
name: edit
description: "AutoSci /edit Codex adapter. Use when the user invokes `/edit` or asks Codex to run AutoSci's edit workflow. Upstream behavior: Add or remove raw sources, or update wiki content, per user request"
---

# AutoSci /edit (Codex Adapter)
...
When this skill is selected, first read `.claude/skills/edit/SKILL.md` completely, then follow
that workflow as the source of truth.

Use these Codex-specific adaptations while following the upstream instructions:

- Treat AutoSci's original slash command `/edit` as this Codex skill.
  If the user writes `/edit ...`, execute it through this skill.
- Resolve any relative references from the upstream skill directory
  `.claude/skills/edit`. For example, `references/foo.md` means
  `.claude/skills/edit/references/foo.md`.
...
- Upstream skill: `.claude/skills/edit/SKILL.md`
