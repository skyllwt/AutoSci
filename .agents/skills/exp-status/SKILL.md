---
name: exp-status
description: "AutoSci /exp-status Codex adapter. Use when the user invokes `/exp-status` or asks Codex to run AutoSci's exp-status workflow. Upstream behavior: View the status of all running experiments; optionally auto-collect completed experiments and advance the pipeline"
---

# AutoSci /exp-status (Codex Adapter)
...
When this skill is selected, first read `.claude/skills/exp-status/SKILL.md` completely, then follow
that workflow as the source of truth.

Use these Codex-specific adaptations while following the upstream instructions:

- Treat AutoSci's original slash command `/exp-status` as this Codex skill.
  If the user writes `/exp-status ...`, execute it through this skill.
- Resolve any relative references from the upstream skill directory
  `.claude/skills/exp-status`. For example, `references/foo.md` means
  `.claude/skills/exp-status/references/foo.md`.
...
- Upstream skill: `.claude/skills/exp-status/SKILL.md`
