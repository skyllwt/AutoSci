...
### Writes
- `wiki/experiments/{slug}.md` — updated via `$exp-run --collect` in Codex or `$exp-run --collect` in Codex during `--collect-ready` mode only when the active agent runtime can invoke skills
...
1. **Determine check scope**:
   - If `--pipeline <slug>` is specified:
     ...
     - If the file does not exist or slug does not match: report error, suggest running `$research` in Codex or `$research` in Codex first, or specifying manually
   - Otherwise:
...
2. **If no running experiments**:
   - Print a friendly message:
     ...
     - To start an experiment: `$exp-run <slug>` in Codex or `$exp-run <slug>` in Codex
...
|-----------|---------|-------|--------|
| [[exp-foo-ablation]] | 0.8h | NaN loss at step 500 | Inspect with `$exp-run exp-foo-ablation --collect` in Codex or `$exp-run exp-foo-ablation --collect` in Codex |
...
1. If running inside Codex and the runtime can invoke another skill workflow, collect each completed experiment sequentially (not in parallel, to avoid concurrent wiki writes):
   ```bash
...
3. **If condition is met (all experiments completed)**:
   ...
   - Append log:
     ...
     ```
   - If running inside Codex and the runtime can invoke another skill workflow, call the next stage:
     ```
...
- **`--pipeline` but pipeline-progress.md does not exist**: report error "Pipeline progress file not found. Run `$research <direction>` in Codex or `$research <direction>` in Codex first, or check wiki/outputs/"
...
### Skills
- `$exp-run` (Codex) or `$exp-run` (Codex) — call collect phase in `--collect-ready` mode only when the active agent runtime can invoke skills
- `$research` (Codex) or `$research` (Codex) — continue Stage 4 via `--auto-advance` only when the active agent runtime can invoke skills
...
- User directly
- `$research` (Codex) or `$research` (Codex) Stage 3b (in interactive mode, suggested to user)
