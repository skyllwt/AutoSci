---
name: review
description: "AutoSci /review Codex adapter. Use when the user invokes `/review` or asks Codex to run AutoSci's review workflow. Upstream behavior: General-purpose cross-model review — Review LLM independently reviews any research artifact, outputs structured scores, wiki entity mapping, and improvement suggestions"
---

# AutoSci /review (Codex Adapter)
...
When this skill is selected, first read `.claude/skills/review/SKILL.md` completely, then follow
that workflow as the source of truth.

Use these Codex-specific adaptations while following the upstream instructions:

- Treat AutoSci's original slash command `/review` as this Codex skill.
  If the user writes `/review ...`, execute it through this skill.
- Resolve any relative references from the upstream skill directory
  `.claude/skills/review`. For example, `references/foo.md` means
  `.claude/skills/review/references/foo.md`.
...
- Upstream skill: `.claude/skills/review/SKILL.md`
