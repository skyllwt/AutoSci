---
name: setup
description: "AutoSci /setup Codex adapter. Use when the user invokes `/setup` or asks Codex to run AutoSci's setup workflow. Upstream behavior: Interactive API key configuration guide — checks current .env state and walks you through Semantic Scholar, DeepXiv, and Review LLM setup"
---

# AutoSci /setup (Codex Adapter)
...
When this skill is selected, first read `.claude/skills/setup/SKILL.md` completely, then follow
that workflow as the source of truth.

Use these Codex-specific adaptations while following the upstream instructions:

- Treat AutoSci's original slash command `/setup` as this Codex skill.
  If the user writes `/setup ...`, execute it through this skill.
- Resolve any relative references from the upstream skill directory
  `.claude/skills/setup`. For example, `references/foo.md` means
  `.claude/skills/setup/references/foo.md`.
...
- Upstream skill: `.claude/skills/setup/SKILL.md`
