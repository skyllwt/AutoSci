---
name: exp-eval
description: "AutoSci /exp-eval Codex adapter. Use when the user invokes `/exp-eval` or asks Codex to run AutoSci's exp-eval workflow. Upstream behavior: Experiment verdict gate — Review LLM independently judges results → 4 verdict paths → auto-update the linked idea's status / failure_reason and graph edges"
---

# AutoSci /exp-eval (Codex Adapter)
...
When this skill is selected, first read `.claude/skills/exp-eval/SKILL.md` completely, then follow
that workflow as the source of truth.

Use these Codex-specific adaptations while following the upstream instructions:

- Treat AutoSci's original slash command `/exp-eval` as this Codex skill.
  If the user writes `/exp-eval ...`, execute it through this skill.
- Resolve any relative references from the upstream skill directory
  `.claude/skills/exp-eval`. For example, `references/foo.md` means
  `.claude/skills/exp-eval/references/foo.md`.
...
- Upstream skill: `.claude/skills/exp-eval/SKILL.md`
