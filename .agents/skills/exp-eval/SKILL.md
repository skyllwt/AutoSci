...
> Manual: `$exp-eval <experiment-slug>` in Codex or `$exp-eval <experiment-slug>` in Codex.
...
3. **Suggest next steps**: `$paper-plan {linked-idea}` in Codex or `$paper-plan {linked-idea}` in Codex, or continue ablation/robustness experiments
...
3. **Suggest supplementary experiments**:
   ...
   - Suggest using `$exp-design --linked-idea {linked-idea}` in Codex or `$exp-design --linked-idea {linked-idea}` in Codex to design supplementary experiments
...
3. **Suggest next steps**:
   ...
   - Suggest `$ideate` in Codex or `$ideate` in Codex to generate alternatives
...
- **Only process completed experiments**: experiments with status != completed are refused; prompt user to use `$exp-run` in Codex or `$exp-run` in Codex first.
...
- **`linked_idea` missing**: refuse to proceed; instruct the user to run `$edit` in Codex or `$edit` in Codex to set the experiment's `linked_idea`.
...
- **Results data missing**: if the experiment page's Results section is empty, prompt user to run `$exp-run {slug} --check` in Codex or `$exp-run {slug} --check` in Codex first.
...
### Called by
- `$research` (Codex) or `$research` (Codex) Stage 4 (verdict and iteration stage)
- User directly
