...
4. Print to terminal:
   ...
   ## Next Steps
   - Compile and verify with `$paper-compile paper/` in Codex or `$paper-compile paper/` in Codex
   - Resolve [UNCONFIRMED] citations manually
   - Polish further with `$refine paper/main.tex --focus writing` in Codex or `$refine paper/main.tex --focus writing` in Codex
   ```
...
- **PAPER_PLAN not found**: error; suggest running `$paper-plan` in Codex or `$paper-plan` in Codex first
- **PAPER_PLAN format incomplete**: list missing sections; suggest re-running `$paper-plan` in Codex or `$paper-plan` in Codex
