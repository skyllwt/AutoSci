---
name: paper-draft
description: "AutoSci /paper-draft Codex adapter. Use when the user invokes `/paper-draft` or asks Codex to run AutoSci's paper-draft workflow. Upstream behavior: Draft a LaTeX paper from PAPER_PLAN — write each section from wiki sources + generate figures/tables + BibTeX verification + de-AI polish"
---

# AutoSci /paper-draft (Codex Adapter)
...
When this skill is selected, first read `.claude/skills/paper-draft/SKILL.md` completely, then follow
that workflow as the source of truth.

Use these Codex-specific adaptations while following the upstream instructions:

- Treat AutoSci's original slash command `/paper-draft` as this Codex skill.
  If the user writes `/paper-draft ...`, execute it through this skill.
- Resolve any relative references from the upstream skill directory
  `.claude/skills/paper-draft`. For example, `references/foo.md` means
  `.claude/skills/paper-draft/references/foo.md`.
...
- Upstream skill: `.claude/skills/paper-draft/SKILL.md`
