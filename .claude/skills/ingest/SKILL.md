...
Skip this step unless the user explicitly passed `--visualize`. Also skip it in INIT MODE — `/init`'s parent process regenerates Canvas once at fan-in, so individual subagents must not duplicate the work and risk concurrent writes.
